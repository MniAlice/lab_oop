 ...
 */
	...
	public void show() {
        ...
        b3.addActionListener(ae -> logic.dataPatientClick(table));
        ...
        buttonPanel.setBackground(new java.awt.Color(90, 90, 90));
        GridBagConstraints buttonGrid = new GridBagConstraints();
        buttonGrid.insets = new Insets(0, 0, 0, 0); // Отступы между кнопками

        // Кнопка save
        buttonGrid.gridx = 0;
        buttonGrid.gridy = 0;
        buttonPanel.add(save, buttonGrid);

        // Кнопка load
        buttonGrid.gridx = 1;
        buttonPanel.add(load, buttonGrid);
        mainGrid.gridx = 0;
        mainGrid.gridy = 0;
        mainGrid.gridwidth = 1;
        mainGrid.anchor = GridBagConstraints.WEST;
        frame.add(buttonPanel, mainGrid);
